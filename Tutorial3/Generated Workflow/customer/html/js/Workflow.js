...
function menuItemCallbackStartFind_Customers() {
    if (!customBeforeMenuItemClick('Start', 'Find_Customers')) {
        return;
    }
    var rmiKeys = [];
    var rmiKeyTypes = [];
    var rmiInputOnlyKeys = [];
    var rmiInputOnlyKeyTypes = [];
    rmiKeys[0] = 'uikCompanyName';
    rmiKeyTypes[0] = 'TEXT';
    rmiInputOnlyKeys[0] = 'uikCompanyName';
    rmiInputOnlyKeyTypes[0] = 'TEXT';
    var workflowMessageToSend = getMessageValueCollectionForOnlineRequest('Start', 'Find_Customers', rmiKeys, rmiKeyTypes);
    var inputOnlyWorkflowMessageToSend = getMessageValueCollectionForOnlineRequest('Start', 'Find_Customers', rmiInputOnlyKeys, rmiInputOnlyKeyTypes);
    if (validateScreen('Start', getCurrentMessageValueCollection(), rmiKeys) && 
        saveScreens(true)) {
        doOnlineRequest('Start', 'Find_Customers', 60, 0, '', null, workflowMessageToSend, inputOnlyWorkflowMessageToSend.serializeToString());
    }
    customAfterMenuItemClick('Start', 'Find_Customers');
}
